...
workflow Mutect2 {
    input {
      ...
      Array[String]? funco_transcript_selection_list
...
task CramToBam {
    ...
    Int machine_mem = select_first([mem, 6000])
...
task SplitIntervals {
    ...
    command {
        ...
        gatk --java-options "-Xmx6000m" SplitIntervals \
            -R ~{ref_fasta} \
            ~{"-L " + intervals} \
...
task M2 {
    ...
    Int machine_mem = select_first([mem, 3500])
...
task MergeVCFs {
    ...
    command {
        set -e
        export GATK_LOCAL_JAR="/root/gatk.jar"
        gatk --java-options "-Xmx6000m" MergeVcfs -I ~{sep=' -I ' input_vcfs} -O ~{output_vcf}
    }
...
task MergeBamOuts {
    ...
    command <<<
        ...
        set -e
        export GATK_LOCAL_JAR="/root/gatk.jar"
        gatk --java-options "-Xmx6000m" GatherBamFiles \
        ...
        gatk --java-options "-Xmx6000m" SortSam -I unsorted.out.bam \
            -O ~{output_vcf_name}.out.bam \
            --SORT_ORDER coordinate -VALIDATION_STRINGENCY LENIENT
        gatk --java-options "-Xmx6000m" BuildBamIndex -I ~{output_vcf_name}.out.bam -VALIDATION_STRINGENCY LENIENT
    >>>
...
task MergeStats {
    ...
    command {
        ...
        gatk --java-options "-Xmx6000m" MergeMutectStats \
...
task MergePileupSummaries {
    ...
    command {
        ...
        gatk --java-options "-Xmx6000m" GatherPileupSummaries \
...
task CalculateContamination {
    ...
    command {
        ...
        gatk --java-options "-Xmx6000m" CalculateContamination -I ~{tumor_pileups} \
...
task Filter {
    ...
    command {
        ...
        gatk --java-options "-Xmx6000m" FilterMutectCalls -V ~{unfiltered_vcf} \
            -R ~{ref_fasta} \
            -O ~{output_vcf} \
...
task Funcotate {
     ...
     command <<<
         ...
         # Run Funcotator:
         gatk --java-options "-Xmx6000m" Funcotator \
