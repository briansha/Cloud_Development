...
mutect2_tumor_reads=()
for (( i=70; i<120; i++ )); do
    mutect2_tumor_reads+=("${args[${i}]}")
done

mutect2_tumor_reads_prefix=()
for (( i=120; i<170; i++ )); do
    mutect2_tumor_reads_prefix+=("${args[${i}]}")
done
mutect2_tumor_reads_name=()
for (( i=170; i<220; i++ )); do
    mutect2_tumor_reads_name+=("${args[${i}]}")
done

# ---------Required Output---------
filter_output_vcf=()
for (( i=220; i<270; i++ )); do
    filter_output_vcf+=("${args[${i}]}")
done
...
do
    ...
    export mutect2_output_basename="${mutect2_tumor_reads_prefix[i]}"
    ...
    export m2_tumor_bam="${mutect2_tumor_reads[i]}"
    #export m2_tumor_bai="${mutect2_tumor_reads_index[i]}" # Don't need this since the path to the cram files was mounted onto the Docker container and those paths contain both the cram and crai file.
    ...
    export filter_filtered_vcf="${filter_output_vcf[i]}"
