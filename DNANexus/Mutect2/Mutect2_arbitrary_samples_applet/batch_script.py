...
destination=sys.argv[3]          # workflow_submissions/mutect2/TODAYSDATE/arbitrary_samples/
batch_execution_tag=sys.argv[4]  # 200K_exome
batch_size=int(sys.argv[5])      # Number of samples per job to use (< 50)
...
for batch_number in range(number_of_batch):
    ...
    print('dx run {applet_id} --input-json-file ../Mutect2_50_samples_applet/workflow_input.json \
     ...
     -y --brief'.format(applet_id=applet_id,batch_mapped_files_cram=batch_mapped_files_cram,batch_mapped_files_crai=batch_mapped_files_crai,batch_number=batch_number,final_folder=final_folder,instance_type=instance_type, batch_execution_tag=batch_execution_tag))
